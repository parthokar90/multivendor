      ...
      <div class="col-xl-4 col-4">
       ...
       <div class="card">
            <div class="card-body" style="height:300px;">
               ...
               <div><b>(-)Coupon:</b>@if(isset($order->coupon->type)) @if($order->coupon->type==1) {{optional($order->coupon)->amount}} %  @php $couponAmount=$subTotals/100*optional($order->coupon)->amount; @endphp  @else {{number_format(optional($order->coupon)->amount)}} Tk @php $couponAmount=$subTotals-$order->coupon->amount; @endphp  @endif  @endif</div> 
               <div><b>(+)Delivery Charge:</b> {{number_format(optional($order->deliveryCharge)->amount)}} Tk </div> 
               <div><b>Grand Total:</b> {{number_format($subTotals-$couponAmount+optional($order->deliveryCharge)->amount)}} Tk</div> 
